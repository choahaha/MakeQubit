-- ============================================================
-- code_runs에 회로 구조 저장 + 개념 오류 분석용 뷰
-- 005 실행 후에 실행할 것
--
-- counts만으로는 '왜 틀렸는지'가 안 보인다. 00과 11 대신 01과 10이 나왔다는
-- 사실은 알아도, CNOT 방향이 뒤집힌 건지 H가 다른 큐비트에 붙은 건지는
-- 게이트 시퀀스를 봐야 안다. 실행마다 회로 구조를 같이 남긴다.
-- ============================================================

alter table public.code_runs
  add column if not exists circuit_spec jsonb;
