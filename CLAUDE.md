...
- **레슨 내용을 고쳐도 학생 화면은 바로 안 바뀐다.** 편집기 초안이
  localStorage에 남아 있기 때문이다. 시작 코드가 바뀌면 알림이 뜨고
  학생이 고를 수 있다 (`showStarterChanged`). 이 처리를 지우지 말 것 —
  지우면 고친 내용이 이미 시작한 학생에게 영영 전달되지 않는다.
