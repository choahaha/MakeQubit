...
8. `backend/sql/008_assessments.sql` — 차시별 형성평가 응답
...
| `assessment_responses` | 형성평가 문항별 응답 — 고른 선택지, 정답 여부, 소요 시간 |
...
차시마다 문항 3개(`data/assessments.json`). 유형이 고정돼 있다:

| 유형 | 무엇을 보는가 |
|---|---|
| 회로 이해 | 코드와 회로를 읽을 수 있는가 (`QuantumCircuit(1, 1)`의 뒤 숫자는?) |
| 결과 예측 | 실행 전에 결과를 예상할 수 있는가 (H 걸고 재면?) |
| 오류 수정 | 잘못된 코드에서 무엇이 문제인지 짚을 수 있는가 |

차시 마지막 레슨을 제출하면 "형성평가 풀기" 버튼이 뜨고, 목차의 차시 끝에도
카드로 놓인다. 그 차시 레슨을 하나도 안 했으면 잠겨 있다 — 배우기 전에 풀면
형성평가가 아니라 사전검사가 된다.

한 문항에 한 번만 답할 수 있다. 고쳐 찍게 두면 정답률이 무의미해진다.
답을 고르면 바로 정오와 해설이 나온다 — 형성평가의 목적이 피드백이기 때문이다.

문항을 고치거나 늘릴 때는 **정답이 실제로 맞는지 시뮬레이터로 확인할 것.**
`결과 예측` 유형은 특히 그렇다. 지금 문항 13개는 전부 실행해서 대조했다.

문항 뒤에 **되돌아보기 서술 하나**가 붙는다 (`assessments.json`의 `reflection`).
차시당 하나뿐이고 건너뛸 수 있다.

원래는 레슨마다 제출할 때 물었다 (16개 중 12개). 제출 버튼을 누른 순간은
학생이 넘어가고 싶은 순간이라 거기서 글을 요구하면 "몰라요"가 돌아온다 —
그건 데이터가 아니라 잡음이다. 형성평가 직후는 방금 무엇을 틀렸는지 본
상태라 쓸 거리가 가장 많은 자리다. **레슨 쪽에 `reflection`을 다시 넣지 말 것.**

저장은 001의 `reflections` 테이블을 재사용한다. `lesson_id` 자리에 `w1s1`
형태로 차시가 들어간다. 분석할 때는 `lesson_id like 'w%s%'`로 거른다.

분석 뷰: `v_item_difficulty`(문항별 정답률, 낮은 순 정렬), `v_item_distractors`
(어떤 오답을 골랐는가 — 오개념 단서), `v_type_mastery`(유형별 성취).
